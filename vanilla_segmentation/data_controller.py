...
class SegDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_noise:
            ...
            rgb = np.array(
                self.trancolor(Image.open('{0}/{1}-color.png'.format(self.root, self.path[index])).convert("RGB")))
        ...
        if self.path[index][:8] == 'data_syn':
            ...
            back = np.array(
                self.trancolor(Image.open('{0}/{1}-color.png'.format(self.root, self.path[seed])).convert("RGB")))
        ...
        return rgb, target, self.path[index]
